...
class Grid:
    def __init__(self):
        ...
        self.states = list((x, y) for x in range(self.x_size)
    ...
    def get_transition_probabilities(self, s, a):
        ...
        for action, probability in self.stoch_action(a).items():
            ...
            probabilities[next_state] = probabilities.get(
                next_state, 0) + probability
...
class PolicyIteration:
    def __init__(self, grid):
        ...
        # t model (lin alg)
        self.t_model = np.zeros([len(self.grid.states), len(
            self.grid.actions), len(self.grid.states)])
        for i, s in enumerate(self.grid.states):
            for j, a in enumerate(self.grid.actions):
                ...
                for next_state, prob in transitions.items():
                    self.t_model[i][j][self.grid.states.index(
                        next_state)] = prob
    ...
    def policy_evaluation(self):
        if not self.USE_LIN_ALG:
            ...
            state_numbers = np.array(range(len(self.grid.states)))
            # print("state numbers", state_numbers)
            # print("self.t_model", self.t_model)
            # print("self.la_policy", self.la_policy)
            t_pi = self.t_model[state_numbers, self.la_policy]
            # print("=====================")
            # print("t_pi", t_pi)
            print("r_model", self.r_model)
            values = np.linalg.solve(np.identity(
                len(self.grid.states)) - (self.grid.discount * t_pi), self.r_model)
            self.values = {s: values[i]
                           for i, s in enumerate(self.grid.states)}
    ...
    def policy_improvement(self):
        if self.USE_LIN_ALG:
            new_policy = {s: self.grid.actions[self.la_policy[i]]
                          for i, s in enumerate(self.grid.states)}
        else:
            new_policy = {}
